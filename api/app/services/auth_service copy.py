...
class AuthService:
    ...
    async def check_user_exists(self, email: str) -> bool:
        ...
        try:
            # 1) Prefer your own users table (fast + cheap)
            result = self.supabase.table('users').select('uid').eq('email', email).limit(1).execute()
            if result.data and len(result.data) > 0:
                return True

            # 2) Fallback: check Supabase Auth (admin). list_users() returns a response object with a `.users` attribute.
            try:
                admin_resp = self.admin_supabase.auth.admin.list_users(page=1, per_page=200)
                users = getattr(admin_resp, "users", []) or []
            except Exception as inner_e:
                print(f"[check_user_exists] admin.list_users failed: {inner_e}")
                users = []

            for u in users:
                try:
                    if getattr(u, "email", None) == email and getattr(u, "email_confirmed_at", None):
                        return True
    ...
    async def send_registration_otp(self, email: str) -> Dict[str, str]:
        ...
        try:
            ...
            try:
                print(f"[send_registration_otp] calling sign_in_with_otp for email={email}")
                ...
                print(f"[send_registration_otp] sign_in_with_otp OK for email={email}")
            except Exception as se:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"supabase sign_in_with_otp error: {str(se)}"
                )
    ...
    async def send_login_otp(self, email: str) -> Dict[str, str]:
        ...
        try:
            ...
            try:
                print(f"[send_login_otp] calling sign_in_with_otp for email={email}")
                response = self.supabase.auth.sign_in_with_otp({
                    "email": email,
                    "options": {
                ...
                print(f"[send_login_otp] sign_in_with_otp OK for email={email}")
            except Exception as se:
                # Surface exact error as 400 so curl shows a message instead of an empty reply
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"supabase sign_in_with_otp error: {str(se)}"
                )
