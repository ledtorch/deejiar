...
export default {
  components: { FormString, FormArray, FormItem },
  data() {
    return {
      ...
      // 🧱 Buidling
      jsonName: import.meta.env.VITE_TEST_NAME,
    };
  },
  computed: {
    totalPages() {
  ...
  methods: {
    ...
    async readName() {
      try {
        const response = await axios.get(this.jsonName);
        console.log('Names of JSON: ', this.jsonName)
      } catch (error) {
        console.error('Failed to fetch jsonName:', error);
      }
    },
