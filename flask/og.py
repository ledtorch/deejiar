...
def detail(title):
    ...
    bots = ['facebookexternalhit', 'twitterbot', 'linkedinbot']  # Extend as needed
    if any(bot in user_agent for bot in bots):
        image_url = get_image_url_for_title(title)
        # Assuming you have a template set up for bots with Open Graph meta tags
        return render_template('bot_response.html', image_url=image_url, title="Deejiar", description="The App.")
    else:
        # Redirect to your SPA's entry point or handle differently
